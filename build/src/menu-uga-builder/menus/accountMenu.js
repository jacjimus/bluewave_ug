...
const accountMenu = (args, db) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    let userPolicy = yield db.policies.findOne({
        where: {
            phone_number: phoneNumber,
            policy_status: "paid"
        },
    ...
    if (currentStep == 1) {
        ...
        switch (userText) {
            ...
            case "4":
                ...
                let beneficiaries = yield db.beneficiaries.findAll({
                    where: {
            ...
            case "5":
                ...
                break;
            case "6":
                let dependants = yield db.beneficiaries.findAll({
                    where: {
                        principal_phone_number: trimmedPhoneNumber,
            ...
            case "7":
                ...
                let user_dependants = yield db.beneficiaries.findAll({
                    where: {
                        principal_phone_number: trimmedPhoneNumber,
                        beneficiary_type: "DEPENDANT"
                    },
                ...
                let dependants_name = user_dependants.map((dependant, index) => {
                ...
                sendSMS_1.default.sendSMS(2, smsPhone, details_message);
                break;
            default:
        ...
        if (userText == "1" && allSteps[1] == '1') {
            ...
            sendSMS_1.default.sendSMS(2, smsPhone, claim_message);
        }
            ...
            if (allSteps[1] == '6' && userText == '1') {
                ...
                if (dependants.length > 0) {
                    response = `CON Your dependants are ${dependants.map((dependant, index) => {
        ...
        if (userText == "1" && allSteps[0] == '1') {
            ...
            let dob = (0, moment_1.default)(allSteps[3], "DD/MM/YYYY").format("YYYY-MM-DD");
            ...
            yield (0, aar_1.updateMember)(data);
            ...
            sendSMS_1.default.sendSMS(2, smsPhone, dob_message);
        }
            ...
            yield db.beneficiaries.create({
                beneficiary_id: (0, uuid_1.v4)(),
                full_name: allSteps[3],
                beneficiary_type: "DEPENDANT",
                user_id: currentUser.user_id,
                dob: (0, moment_1.default)('01/01/1990', "DD/MM/YYYY").format("YYYY-MM-DD"),
                principal_phone_number: trimmedPhoneNumber
            });
            ...
            sendSMS_1.default.sendSMS(2, smsPhone, dependant_message);
        ...
        if (allSteps[1] == "4" && allSteps[0] == "4") {
            ...
            let beneficiary;
            if (beneficiaries.length > 0) {
                beneficiary = yield db.beneficiaries.update({
                    full_name: allSteps[3],
                ...
                beneficiary = yield db.beneficiaries.create({
                    beneficiary_id: (0, uuid_1.v4)(),
                    full_name: allSteps[3],
                    first_name: allSteps[3].split(" ")[0],
                    last_name: allSteps[3].split(" ")[1],
            ...
            sendSMS_1.default.sendSMS(2, smsPhone, kin_message);
