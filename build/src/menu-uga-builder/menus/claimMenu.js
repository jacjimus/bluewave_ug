    ...
    if (currentStep === 1) {
        ...
        switch (userText) {
            case "1":
                ...
                policy = yield db.policies.findOne({
                    where: {
        ...
        let user = yield db.users.findOne({
            where: {
                [db.Sequelize.Op.or]: [
                    ...
                    { phone_number: (_b = args.phoneNumber.replace('+', "")) === null || _b === void 0 ? void 0 : _b.substring(3) },
                ]
            },
            limit: 1,
        });
        ...
        const existingClaim = yield db.claims.findAll({
            where: {
                user_id: user.user_id,
                claim_status: "pending",
        ...
        yield sendSMS_1.default.sendSMS(2, `+256${user.phone_number}`, sms);
        ...
        yield db.claims.create({
            ...
            claim_status: "pending",
            partner_id: user.partner_id,
            claim_description: `Death Claim ID: ${claimId} for AAR Memberr Numbe: ${user.arr_membership_number}  ${policy[policy.length - 1].policy_type.toUpperCase()} ${policy[policy.length - 1].beneficiary.toUpperCase()} policy`,
            claim_type: "Death Claim",
