...
const countPoliciesByInstallementOrder = (policies: any[], installment_order: number): number => {
    return policies.filter((policy: any) => policy.installment_order === installment_order).length;
};
...
const getPolicyExcelReportDownload = async (req, res) => {
  ...
  const generatePolicyExcelReport = async (policies) => {
    ...
    worksheet.columns = [
        ...
        { header: 'Policy Deduction Amount', key: 'policy_deduction_amount', width: 20 },
        { header: 'Policy Next Deduction Date', key: 'policy_next_deduction_date', width: 20 },
        ...
        { header: 'Installment Alert Date', key: 'installment_alert_date', width: 20 },
