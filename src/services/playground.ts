...
async function processUsersPolicyAARWithAANumber() {
  ...
  for (const member of arrMembersWithnoPolicy) {
    ...
    try {
      ...
      console.log("===== USER ====",user.name, user.phone_number, policy.policy_number);
     let updated_premium = await  updatePremium(user, policy)
    
      count++
      // Add a delay between iterations (adjust the delay time as needed)
      console.log(`Dependant created for user with phone number: ${user.phone_number}, response: ${updated_premium}`);
      await delay(2000);
      console.log(count)
    } catch (error) {
  ...
  try {
    ...
    const policies = await db.policies.findAll({
      where: {
        ...
        installment_type: 2,
        policy_start_date: {
          [Op.between]: [new Date('2021-09-13'), new Date('2021-09-30')]
        },
      }
    });
    console.log('policies', policies.length)
...
async function userWithoutAARmemberNumber() {
  ...
  userPhone.forEach(async (phone) => {
    ...
    console.log("USER", user.name, user.phone_number)

    if (user) {
      let policy = await db.policies.findOne({
        where: {
          [Op.or]: [
            { phone_number: `+256${user.phone_number}` },
            { user_id: user.user_id },
          ],
          policy_status: 'paid',
        },
      });
      console.log("POLICY", policy?.policy_number, policy?.first_name, user.name)

      if (policy) {
      let memberDetails = await  getMemberNumberData(`256${user.phone_number}`);
      console.log("MEMBER DETAILS", memberDetails)
      
    let regMember = await  registerPrincipal(user, policy);
   console.log("MEMBER DETAILS", regMember)
      // if(memberDetails.code == 200){
      // user.arr_member_number = memberDetails.member_number;
      // user.save();

      //   console.log("POLICY",user.arr_member_number, policy.policy_number)
      //   //const updatePremiumData = await updatePremium(user, policy);
      //   //console.log("AAR UPDATE PREMIUM - updated", updatePremiumData);
      // }else{
      //   console.log("MEMBER NUMBER NOT FOUND", user.name, user.phone_number)
      // }
    }
...
//update policy status to paid if the policy have a paid payment of that amount

async function updatePolicyStatusToPaid() {
  let allPaidPolicies = await db.policies.findAll({
    where: {
      policy_status: 'unpaid',
      partner_id: 2,
    },
  });

  allPaidPolicies.forEach(async (policy) => {
    try {
      // const payment = await db.payments.findOne({
      //   where: {
      //     policy_id: policy.policy_id,
      //     payment_status: 'paid',
      //     payment_amount: policy.premium,
      //   },
      // });

      // if (payment) {
      console.log("POLICY", policy.first_name, policy.policy_number)
      await db.policies.update(
        { policy_status: 'pending' },
        { where: { policy_id: policy.policy_id } }
      );
...
async function updateAARpremium() {
  for (const member of member_no) {
    try {
      const user = await db.users.findOne({
        where: {
         [Op.or]: [
            { arr_member_number: member },
            { arr_member_number: member.slice(0, 8) + "-" + "00" },
          ],
          partner_id: 2,
        },
      });

      console.log("USER", user?.name, member, user?.phone_number)
      ...
      if(policy?.beneficiary == 'FAMILY' && policy){
        console.log("POLICY",policy, policy?.policy_number, policy?.first_name, user?.name)
      // CREATE DEPENDANTS
        let dep = await createDependant(user, policy);
        console.log('FAMILY DEPENDANT - member created', dep);
        console.log("AAR DEPENDANT - member created", dep);
      }else if (policy?.beneficiary == 'SELF' && policy.beneficiary =='OTHER'){ 
        if(policy.policy_status == 'paid' && policy){
           await updatePremium(user, policy);
        console.log("SELF OR OTHER POLICY", user?.name, user?.phone_number)
      }
    }else{
      console.log("NO POLICY", user?.name, user?.phone_number)
    }
      // if (policy && user.arr_member_number == member) {
      //   console.log("POLICY", policy.first_name, policy.last_name, policy.policy_number, policy.premium);
      //   await updatePremium(user, policy);
      // }

      // Introduce a delay between updates (e.g., 1000 milliseconds = 1 second)
